...
class CrowdSim_mixed(gym.Env):
    ...
    def reset(self,non_attentive_humans, phase='test', test_case=None):
        """
        ...
        if self.robot.sensor == 'coordinates':
            ob = self.compute_observation_for(self.robot, non_attentive_humans)
    ...
    def step(self, action, non_attentive_humans, update=True):
        ...
        if self.centralized_planning:
            ...
            #     ob = self.compute_observation_for(human, non_attentive_humans)
            #
            #     human_actions.append(human.act(ob))
    ...
    def compute_observation_for(self, agent, non_attentive_humans):
        if agent == self.robot:
            ob = []
            for human in self.humans:
    ...
    def render(self,case,policy,environment,human_number,trained_env, mode='video', output_file=None):
        ...
        if mode == 'traj':
            ...
            if output_file is not None:
                ...
                writer = ffmpeg_writer(fps=10, metadata=dict(artist='Me'), bitrate=1800)
                anim.save(f"/home/fbaldini/Desktop/All_Videos/policy_{policy}_trained_env_{trained_env}_human_environment_{environment}_human_number_{human_number}_case_{case}.mp4", writer=writer)
